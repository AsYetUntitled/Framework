...
if (isNil {uiNamespace getVariable "life_sql_id"}) then {
    ...
    uiNamespace setVariable ["life_sql_id",life_sql_id];
        try {
        if (!((EXTDB format ["9:ADD_DATABASE:%1",EXTDB_SETTING(getText,"DatabaseName")]) isEqualTo "[1]")) then {throw "extDB3: Error with Database Connection"};
        if (!((EXTDB format ["9:ADD_DATABASE_PROTOCOL:%2:SQL:%1:TEXT2",FETCH_CONST(life_sql_id),EXTDB_SETTING(getText,"DatabaseName")]) isEqualTo "[1]")) then {throw "extDB3: Error with Database Connection"};
    } catch {
        diag_log _exception;
