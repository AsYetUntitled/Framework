...
switch (_side) do
{
	case west:
	{
		...
		*/
		_query = format["SELECT playerid, name, money, bank, coplevel, cop_licenses, cop_gear, adminlevel FROM players WHERE playerid='%1'",_uid];
	};
	...
	case civilian:
	{
		...
		*/
		_query = format["SELECT playerid, name, money, bank, civ_licenses, arrested, adminlevel, donatorlvl FROM players WHERE playerid='%1'",_uid];
	};
};
diag_log format["SQL QUERY: UID - %1",_uid];
	...
	else
{
	...
	switch(_side) do
	{
		case west:
		{
			diag_log format["SQL QUERY: COP GEAR - %1",(_ret select 6)];
