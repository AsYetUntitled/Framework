...
diag_log format["REQUEST: %1", _this];
...
switch (_side) do
{
	case west:
	{
		...
		_query = format["INSERT INTO players (playerid, name, money, bank, cop_gear, aliases) VALUES('%1', '%2', '%3', '%4', '%5', '%6')",_uid,_name,_money,_bank,_misc,_alias];
	};
	
	case civilian:
	{
		if(typeName _misc == "BOOL") then {_misc = [_misc] call DB_fnc_bool;};
		_query = format["INSERT INTO players (playerid, name, money, bank, arrested, aliases) VALUES('%1', '%2', '%3', '%4', '%5', '%6')",
		_uid, //1
		_name, //2 
		_money,//3
		_bank, //4
