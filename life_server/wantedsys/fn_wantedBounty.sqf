...
if(_id != -1) then
{
	if(_half) then
	{
		[[((life_wanted_list select _id) select 3) / 2,((life_wanted_list select _id) select 3)],"fnc_recv_bounty",(owner _cop),false] spawn BIS_fnc_MP;
	}
		else
	{
		//[(life_wanted_list select _id) select 3,(life_wanted_list select _id) select 3] call fnc_recv_bounty;
		[[(life_wanted_list select _id) select 3,(life_wanted_list select _id) select 3],"fnc_recv_bounty",(owner _cop),false] spawn BIS_fnc_MP;
	};
};