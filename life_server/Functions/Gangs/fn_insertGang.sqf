...
private _queryResult = [format ["SELECT id FROM gangs WHERE name='%1' AND active='1'",_gangName],2] call DB_fnc_asyncCall;
...
_queryResult = [format ["SELECT id FROM gangs WHERE members LIKE '%2%1%2' AND active='1'",_uid,"%"],2] call DB_fnc_asyncCall;
...
_queryResult = [format ["SELECT id, active FROM gangs WHERE name='%1' AND active='0'",_gangName],2] call DB_fnc_asyncCall;
...
_queryResult = [format ["SELECT id FROM gangs WHERE owner='%1' AND active='1'",_uid],2] call DB_fnc_asyncCall;
