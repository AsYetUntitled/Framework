...
if (!(count ([format ["SELECT id FROM gangs WHERE name='%1' AND active='1'",_gangName],2] call DB_fnc_asyncCall) isEqualTo 0)) exitWith {
...
if (!(count ([format ["SELECT id FROM gangs WHERE members LIKE '%2%1%2' AND active='1'",_uid,"%"],2] call DB_fnc_asyncCall) isEqualTo 0)) exitWith {
...
if (!(count ([format ["SELECT id, active FROM gangs WHERE name='%1' AND active='0'",_gangName],2] call DB_fnc_asyncCall) isEqualTo 0)) then {
    [format ["UPDATE gangs SET active='1', owner='%1',members='%2' WHERE id='%3'", _uid, [[_uid]] call DB_fnc_mresArray, (([format ["SELECT id, active FROM gangs WHERE name='%1' AND active='0'",_gangName],2] call DB_fnc_asyncCall) select 0)],1] call DB_fnc_asyncCall;
} else {
    [format ["INSERT INTO gangs (owner, name, members) VALUES('%1','%2','%3')", _uid, _gangName, [[_uid]] call DB_fnc_mresArray],1] call DB_fnc_asyncCall;
};
...
_group setVariable ["gang_id",(([format ["SELECT id FROM gangs WHERE owner='%1' AND active='1'",_uid],2] call DB_fnc_asyncCall) select 0),true];
