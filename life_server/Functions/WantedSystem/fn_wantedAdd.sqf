...
private _queryResult = [format ["SELECT wantedID FROM wanted WHERE wantedID='%1'",_uid],2,true] call DB_fnc_asyncCall;
private _val = [_type select 1] call DB_fnc_numberSafe;
private _number = _type select 0;

if !(count _queryResult isEqualTo 0) then {
    _queryResult = [format ["SELECT wantedCrimes, wantedBounty FROM wanted WHERE wantedID='%1'",_uid],2] call DB_fnc_asyncCall;
    ...
    [format ["UPDATE wanted SET wantedCrimes = '%1', wantedBounty = wantedBounty + '%2', active = '1' WHERE wantedID='%3'",_pastCrimes,_val,_uid],1] call DB_fnc_asyncCall;
} else {
    _crime = [_type select 0];
    _crime = [_crime] call DB_fnc_mresArray;
    [format ["INSERT INTO wanted (wantedID, wantedName, wantedCrimes, wantedBounty, active) VALUES('%1', '%2', '%3', '%4', '1')",_uid,_name,_crime,_val],1] call DB_fnc_asyncCall;
};
