...
if(EQUAL(LIFE_SETTINGS(getNumber,"player_moneyLog"),1)) then {
    if(LIFE_SETTINGS(getNumber,"battlEye_friendlyLogging") isEqualTo 1) then {
        money_log = format ["transfered %1 to %2. Bank Balance: %3  On Hand Balance: %4",_value,_unit getVariable ["realname",name _unit],[BANK] call life_fnc_numberText,[CASH] call life_fnc_numberText];
    } else {
        money_log = format ["%1 - %2 transfered %3 to %4. Bank Balance: %5  On Hand Balance: %6",profileName,(getPlayerUID player),_value,_unit getVariable ["realname",name _unit],[BANK] call life_fnc_numberText,[CASH] call life_fnc_numberText];
    };
