...
if (count _invs > 0) then {
    {
        private _displayName = M_CONFIG(getText,"VirtualItems",(_x select 0),"displayName");
        private _inv = _inv + format ["%1 %2<br/>",(_x select 1),(localize _displayName)];
        private _price = M_CONFIG(getNumber,"VirtualItems",(_x select 0),"sellPrice");
