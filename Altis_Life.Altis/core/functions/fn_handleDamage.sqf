...
{
	_damage = false;
	if(!life_istazed && !(player getVariable "restrained") && player distance _source < 35 && vehicle player == player) then
	{
...
{
	_damage = false;
	if(!life_istazed && !(player getVariable "restrained") && player distance _source < 70 && vehicle player == player) then
	{
		player allowDamage false;
		[_unit,_source] spawn life_fnc_tazed;
	};
