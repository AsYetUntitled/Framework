...
if (str (_target) in LIFE_SETTINGS(getArray,"delivery_points")) then {
    ...
    private _deliveryPoint = _point call BIS_fnc_selectRandom;
} else {
    private _deliveryPoint = LIFE_SETTINGS(getArray,"delivery_points") call BIS_fnc_selectRandom;
};

life_dp_start = _target;

life_delivery_in_progress = true;
life_dp_point = call compile format ["%1", _deliveryPoint];

_deliveryPoint = [_deliveryPoint, "_", " "] call KRON_Replace;
